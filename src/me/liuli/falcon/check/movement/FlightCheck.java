...
//                return new CheckResult("tried to fly on the Y-axis (mY=" + movementCache.motionY + ", max=" + maxMotionY + ")");
            ...
            if (Math.abs(movementCache.motionY
                ...
                String result = "had too little Y dropoff (diff=" + Math.abs(movementCache.motionY - movementCache.lastMotionY) + ")";
                SakuraAPIAB api = new SakuraAPI();
                api.addVL(player, "SurvivalFly", 1.2d, result);
        ...
        if (movementCache.lastMotionY > 0 && movementCache.motionY > 0 && movementCache.airTicks == 2
            ...
            String result = "tried to climb air (mY=" + movementCache.motionY + ")";
            SakuraAPIAB api = new SakuraAPI();
            api.addVL(player, "SurvivalFly", 1.2d, result);
            return new CheckResult(result);
        }
        ...
        if (movementCache.motionY > 0.42 && movementCache.airTicks > 2
                ...
                && movementCache.slimeInfluenceTicks == 0 && movementCache.elytraEffectTicks <= 25) {
            String result = "tried to climb air (mY=" + movementCache.motionY + ", at=" + movementCache.airTicks + ")";
            SakuraAPIAB api = new SakuraAPI();
            api.addVL(player, "SurvivalFly", 1.2d, result);
            return new CheckResult(result);
        }
        ...
        if ((!realOnGround) && movementCache.lastOnGround) {
            String result = "faked ground to fly (og=" + player.onGround + ", rog=" + realOnGround + ")";
            SakuraAPIAB api = new SakuraAPI();
            api.addVL(player, "SurvivalFly", 1.2d, result);
            return new CheckResult(result);
        }
        ...
        if (!movementCache.onGround && movementCache.motionY < 0
            ...
            if (MoveUtil.isNearBlock(distance.getFrom().clone().subtract(0, 0.51D, 0), Block.LADDER)
                ...
                maxOffset += 0.15D;
            if (offset > maxOffset) {
                String result = "ignored gravity (offset=" + offset + ", at=" + movementCache.airTicks + ")";
                SakuraAPIAB api = new SakuraAPI();
                api.addVL(player, "SurvivalFly", 1.2d, result);
                return new CheckResult(result);
            }
